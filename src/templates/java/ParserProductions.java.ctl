...
#function resolveTreeNodeBehavior expansion
   #var treeNodeBehavior = expansion::treeNodeBehavior,
      ...
      #endif
   #endif
   #if (treeNodeBehavior!::neverInstantiated)!
...
#function getLhsPattern assignment, lhsType
      ...
      #var lhsName = assignment::name
         ...
         #set lhsName = capFirst(lhsName)
            ...
            #return globals.translateIdentifier("THIS") + ".get" + lhsName + "().add(" + getRhsAssignmentPattern(assignment) + ")"
         #else
            #-- This is an assignment of the current node's effective value to the specified property of the production node --
            #return globals.translateIdentifier("THIS") + ".set" + lhsName + "(" + getRhsAssignmentPattern(assignment) + ")"
         #endif
...
#macro BuildCodeFailure fail
      ...
      pushOntoCallStack("${fail::containingProduction::name}",
                        "${fail::inputSource.JavaStringEncode}",
   ...
   #var assertionMessage = "Assertion at: " + assertion::location.JavaStringEncode + " failed. "
       ...
       )
       {
         pushOntoCallStack("${assertion::containingProduction::name}",
                        "${assertion::inputSource.JavaStringEncode}",
...
#macro AcceptNonTerminal nonterminal
   #var lhsClassName = nonterminal::production::nodeName,
      ...
      #set impliedLHS = globals.translateIdentifier("THIS") + "." + imputedJtbFieldName(nonterminal::production::nodeName) + " = @"
   #endif
...
#macro BuildCodeTerminal terminal
   ...
   #var lambda = terminal::recoveryBlock!
   #if !settings::faultTolerant
       pushOntoCallStack("${currentProduction::name}","${terminal::inputSource.JavaStringEncode}", ${terminal::beginLine}, ${terminal::beginColumn});
       try {
       ...
       #endif
       pushOntoCallStack("${currentProduction::name}","${terminal::inputSource.JavaStringEncode})", ${terminal::beginLine}, ${terminal::endLine});
       try {
   ...
   while (true) {
     ${RecoveryLoop(oom, cardinalitiesVar)} [#-- REVISIT: recovery of cardinalities!!! --]
      ...
      if (!${cardinalitiesVar}.checkCardinality(false))  {
         fail("Minimum cardinality constraint(s) for: ${oom::location.JavaStringEncode} not met.", getToken(1));
      }
   #endif
...
#macro BuildCodeZeroOrMore zom
   ...
   while (true) {
      ...
      if (!${cardinalitiesVar}.checkCardinality(false))  {
         fail("Minimum cardinality constraint(s) for: ${zom::location.JavaStringEncode} not met.", getToken(1));
      }
   #endif
#endmacro
