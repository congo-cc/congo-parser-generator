  ...
  [/#if]
{
  ...
  [#if inLookahead]
    ...
    self.active_token_types = _prev_active_${taIdx};
    if _something_changed_${taIdx} {
        let _ = self.retokenize_from_pos();
    }
...
[#else]
        if _reached_scan_code_${scanIdx} && self.remaining_lookahead as i32 <= _threshold_${scanIdx} {
            self.passed_predicate = true;
        }
[/#if]
...
[#macro BuildAssertionRoutine expansion]
    ...
    fn ${expansion.scanRoutineName}(&mut self) -> bool {
        ...
        let _prev_scan_pos_${assertIdx} = self.scan_pos;
        let _prev_hit_failure_${assertIdx} = self.hit_failure;
        if self.scan_pos == 0 {
            self.scan_pos = self.pos;
        }
        let _result_${assertIdx} = (|| -> bool {
            self.lookahead_routine_nesting += 1;
[@BuildScanCode expansion /]
            true
        })();
        self.lookahead_routine_nesting -= 1;
        self.scan_pos = _prev_scan_pos_${assertIdx};
        self.remaining_lookahead = _prev_remaining_${assertIdx};
        self.hit_failure = _prev_hit_failure_${assertIdx};
        _result_${assertIdx}
    }
[/#macro]
...
[@BuildScanCode lookahead.nestedExpansion /]
        ...
        self.lookahead_routine_nesting -= 1;
        self.scan_pos = _prev_scan_${laIdx};
        self.remaining_lookahead = _prev_remaining_${laIdx};
        self.hit_failure = _prev_hit_${laIdx};
        _result_${laIdx}
    }
[/#if]
[/#macro]
