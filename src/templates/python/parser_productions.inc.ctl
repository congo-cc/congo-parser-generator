...
# This code is generated by the parser_productions.inc.ctl template.
# ===================================================================
   ...
   [#if assignment??]
      ...
      [#elif assignment::namedAssignment!false]
         ...
         [#else]
            ...
            [#return "${currentNodeVariableName()}" + ".set_named_child(\"" + globals.translateIdentifier(lhsName) + "\", " + getRhsAssignmentPattern(assignment) + ")" /]
         [/#if]
      [/#if]
...
${globals.translateCodeBlock(fail::code, indent)}[#rt]
     #endif
   #elif fail::exp??
${is}self.fail('Failure: %s' % "${fail::exp.JavaStringEncode}")
   #else
...
${is}    self.fail('Minimum cardinality constraint(s) for: ${oom::location.JavaStringEncode} not met.')
[/#if]
...
${is}if not ${cardinalitiesVar}.check_cardinality(False):
${is}    self.fail('Minimum cardinality constraint(s) for: ${zom::location.JavaStringEncode} not met.')
[/#if]
[#--${is}# DBG < BuildCodeZeroOrMore ${indent} --]
[/#macro]
...
${is}    if self.debug_fault_tolerant:
${is}        logger.info('Repeat re-sync for expansion at: ${loopExpansion::location.JavaStringEncode}');
...
${is}    self.push_onto_call_stack('${currentProduction::name}', '${choice::inputSource.JavaStringEncode}', ${choice::beginLine}, ${choice::beginColumn})
