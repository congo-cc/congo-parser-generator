/* Generated by: CongoCC Parser Generator. DelegatedMultiParentParser.java  */
...
public class DelegatedMultiParentParser { 
    ...
    /** Generated Lexer. */
    private DelegatedMultiParentLexer token_source;
    ...
    //=================================
    public DelegatedMultiParentParser(String inputSource, CharSequence content) { 
        this(new DelegatedMultiParentLexer(inputSource, content));
    }

    public DelegatedMultiParentParser(CharSequence content) { 
        this("input", content);
    }
    ...
    */
    public DelegatedMultiParentParser(String inputSource, Path path) throws IOException { 
        this(inputSource, TokenSource.stringFromBytes(Files.readAllBytes(path)));
    }

    public DelegatedMultiParentParser(String inputSource, Path path, Charset charset) throws IOException { 
    ...
    * @param path The location (typically the filename) from which to get the input to parse
    */
    public DelegatedMultiParentParser(Path path) throws IOException { 
        this(path.toString(), path);
    }

    /** Constructor with user supplied Lexer. */
    public DelegatedMultiParentParser(DelegatedMultiParentLexer lexer) { 
        token_source = lexer;
    ...
    */
    ...
    // DelegatedMultiParent.ccc:5:1
    final public void Root() { 
        if (cancelled) throw new CancellationException();
        this.currentlyParsedProduction = "Root";
        // Code for BNFProduction specified at DelegatedMultiParent.ccc:5:1
        ...
        try { 
            // Code for NonTerminal specified at DelegatedMultiParent.ccc:5:8
            pushOntoCallStack("Root", "DelegatedMultiParent.ccc", 5, 8);
            try { 
                Parent();
            } finally { 
                popCallStack();
    ...
    // DelegatedMultiParent.ccc:7:1
    final public void Parent() { 
        if (cancelled) throw new CancellationException();
        this.currentlyParsedProduction = "Parent";
        // Code for BNFProduction specified at DelegatedMultiParent.ccc:7:1
        ...
        int callStackSize4 = parsingStack.size();
        try { 
            // Code for OneOrMore specified at DelegatedMultiParent.ccc:7:10
            RepetitionCardinality cardinalities0 = new RepetitionCardinality(new int[][] { new int[] { 0,
            1}});
            while (true) { 
                // Code for NonTerminal specified at DelegatedMultiParent.ccc:7:12
                pushOntoCallStack("Parent", "DelegatedMultiParent.ccc", 7, 12);
                pushDelegatedCardinality(cardinalities0, 0);
                try { 
                    Child();
                } finally { 
                    popCallStack();
                ...
                if (!(scan$DelegatedMultiParent_ccc$7$12())) break;
            }
            // Code for ZeroOrMore specified at DelegatedMultiParent.ccc:7:21
            RepetitionCardinality cardinalities1 = new RepetitionCardinality(new int[][] { new int[] { 0,
            1}});
            while (true) { 
                if (!(scan$DelegatedMultiParent_ccc$7$23())) break;
                // Code for NonTerminal specified at DelegatedMultiParent.ccc:7:23
                pushOntoCallStack("Parent", "DelegatedMultiParent.ccc", 7, 23);
                pushDelegatedCardinality(cardinalities1, 0);
                try { 
                    Child();
                } finally { 
                    popCallStack();
                ...
                cardinalities1.commitIteration(true);
            }
        } catch (ParseException e) { 
            parseException3 = e;
            throw e;
    ...
    // DelegatedMultiParent.ccc:9:1
    final public void Child() { 
        if (cancelled) throw new CancellationException();
        this.currentlyParsedProduction = "Child";
        // Code for BNFProduction specified at DelegatedMultiParent.ccc:9:1
        ...
        int callStackSize4 = parsingStack.size();
        try { 
            // Code for Assertion specified at DelegatedMultiParent.ccc:9:9
            if (!peekDelegatedCardinality().choose(peekDelegatedBias() + 0, false)) { 
                fail("Maximum cardinality constraint at: DelegatedMultiParent.ccc:9:9 exceeded.", getToken(1));
            }
            // Code for Terminal specified at DelegatedMultiParent.ccc:9:24
            pushOntoCallStack("Child", "DelegatedMultiParent.ccc", 9, 24);
            try { 
                consumeToken(X);
            } finally { 
                popCallStack();
    ...
    // BuildPredicateRoutine: ExpansionSequence at DelegatedMultiParent.ccc:7:12
    private boolean scan$DelegatedMultiParent_ccc$7$12() { 
        ...
        try { 
            ...
            // Lookahead Code for NonTerminal specified at DelegatedMultiParent.ccc:7:12
            // NonTerminal Child at DelegatedMultiParent.ccc:7:12
            pushOntoLookaheadStack("Parent", "DelegatedMultiParent.ccc", 7, 12);
    ...
    // BuildPredicateRoutine: ExpansionSequence at DelegatedMultiParent.ccc:7:23
    private boolean scan$DelegatedMultiParent_ccc$7$23() { 
        remainingLookahead = 1;
        currentLookaheadToken = lastConsumedToken;
        final boolean scanToEnd = false;
        try { 
            ...
            if (hitFailure) return false;
            if (remainingLookahead <= 0) return true;
            // Lookahead Code for NonTerminal specified at DelegatedMultiParent.ccc:7:23
            // NonTerminal Child at DelegatedMultiParent.ccc:7:23
            pushOntoLookaheadStack("Parent", "DelegatedMultiParent.ccc", 7, 23);
            currentLookaheadProduction = "Child";
            try { 
                if (!check$Child(false, null)) return false;
            } finally { 
    ...
    private boolean check$Child(boolean scanToEnd, RepetitionCardinality cardinalities) { 
        ...
        // Lookahead Code for Assertion specified at DelegatedMultiParent.ccc:9:9
        // Cardinality constraint check to ensure maximum not reached.
        if (cardinalities != null && !cardinalities.choose(peekDelegatedBias() + 0, true)) { 
            hitFailure = true;
            return cardinalities != null ? cardinalities.commit(false) : false;
        }
        if (hitFailure) return cardinalities != null ? cardinalities.commit(false) : false;
        if (remainingLookahead <= 0) return cardinalities != null ? cardinalities.commit(true) : true;
        // Lookahead Code for Terminal specified at DelegatedMultiParent.ccc:9:24
    ...
    private void pushOntoCallStack(String methodName, String fileName, int line, int column) { 
        parsingStack.add(new NonTerminalCall("DelegatedMultiParentParser", getToken(1), fileName, methodName, line, column));
    }
    ...
    private void pushOntoLookaheadStack(String methodName, String fileName, int line, int column) { 
        lookaheadStack.add(new NonTerminalCall("DelegatedMultiParentParser", getToken(1), fileName, methodName, line, column));
    }
    ...
    */
    ...
    class NodeScope extends ArrayList<Node> { 
        ...
        NodeScope() { 
            this.parentScope = DelegatedMultiParentParser.this.currentNodeScope;
            DelegatedMultiParentParser.this.currentNodeScope = this;
        }
        ...
        void close() { 
            ...
            DelegatedMultiParentParser.this.currentNodeScope = parentScope;
        }
