...
import org.parsers.cics.CicsParser;
import org.parsers.cics.Node;

/**
 * A test harness for parsing Cics source code
  */
public class CicsParse {
   ...
   static public void parseFile(File file, boolean dumpTree) throws IOException {
       CicsParser parser = new CicsParser(file.toPath());
   ...
   static public void addFilesRecursively(List<File> files, File file) {
       ...
       else if (file.getName().endsWith(".cics")) {
           files.add(file);
       }
   }

   static public void usage() {
       System.out.println("Usage: java CicsParse <sourcefiles or directories>");
       System.out.println("If you just pass it one Cics source file, it dumps the AST");
       System.exit(-1);
   }
}
